...
namespace MyBot
{
	...
	{
		/// 자동 패배 체크 실행 여부
		bool isToCheckLostCondition;

		/// 자동 패배 체크 결과
		bool isLostConditionSatisfied;

		/// 자동 패배 조건이 시작된 프레임 시점
		int lostConditionSatisfiedFrame;

		/// 자동 패배 조건이 만족된채 게임을 유지시키는 최대 프레임 수
		int maxDurationForLostCondition;

		/// 자동 패배 조건을 체크하여, 조건 만족 시 GG 선언하고 게임을 나갑니다
		void checkLostConditionAndLeaveGame();


		/// 타임 아웃 체크 실행 여부
		bool isToCheckTimeOut;

		/// 타임 아웃 관련 변수들
		int timeOverConditionSatisfiedFrame;
		...
		std::vector<long long> timeStartedAtFrame;
		std::vector<long long> timeElapsedAtFrame;
